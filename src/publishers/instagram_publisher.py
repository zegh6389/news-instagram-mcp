        ...
        try:
            ...
            if session_path.exists():
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"Existing session invalid, will try relogin: {session_test_error}")
                        
                        # Try to relogin with existing session
                        try:
                            self.client.relogin()
                            account_info = self.client.account_info()
                            logger.info("✅ Successfully reused Instagram session after relogin")
                            logger.info(f"📱 Connected as @{account_info.username}")
                            return True
                        except Exception as relogin_error:
                            logger.warning(f"Relogin failed: {relogin_error}")
            ...
            if config.instagram_username and config.instagram_password:
                ...
                try:
                    ...
                    logger.error(f"Fresh login failed: {login_error}")
                    
                    # If fresh login fails, try the session approach one more time
                    if "challenge" in str(login_error).lower() or "ForceAppUpgrade" in str(login_error):
                        logger.warning("⚠️  Instagram requires manual verification")
                        logger.info("💡 Please run: python setup_instagram_session.py")
                        logger.info("💡 This will setup your session using cookies to bypass challenges")
